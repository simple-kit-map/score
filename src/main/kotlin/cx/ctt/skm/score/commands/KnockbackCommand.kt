...
class KnockbackCommand (private val plugin: Score) : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        var uuid = if (sender !is Player){
            Bukkit.getOfflinePlayer(args.removeAt(1)).uniqueId
        } else {
            sender.uniqueId
        }

        var presets = plugin.config.getConfigurationSection("old-player-knockback.custom.configs")?.getKeys(false)
        ...
        if (args.size == 2 && args[0] in KB_DEL.entries.map { it.name }){
            if (sender.hasPermission("deleteotherskb")){
                var path = "old-player-knockback.custom.configs." + args[1]
        ...
        if (args.size == 8 || args.size == 7 || args.size == 9){
            ...
            var path = "old-player-knockback.custom.configs.${args[0]}"
            ...
            var valueDisplay = args.slice(1..args.lastIndex).joinToString(" ")
            plugin.config.set("$path.valuesDisplay", valueDisplay)

            var keys = plugin.config.getConfigurationSection(path)?.getKeys(false)
        ...
        if (args.size == 2 && args[0] == "get"){
            var targetkb = plugin.config.getString("old-player-knockback.custom.selection.${args[1]}")
        ...
        if (args.size == 2 && args[0] in KB_SET.entries.map { it.name }) {
            ...
            var selectedKb = "old-player-knockback.custom.configs.${args[1]}"

            var hitDelay = plugin.config.getInt("$selectedKb.values.hitdelay", 20)
            ...
            if (!plugin.config.contains(selectedKb)){
                sender.sendMessage("Knockback preset ${args[1]} does not exist")
                return false
            }
            var valuesDisplay = plugin.config.getString("$selectedKb.valuesDisplay")

            var selectionPath = "old-player-knockback.custom.selection.$uuid"

            var previousPreset = plugin.config.getString(selectionPath)

            var previousString = if (previousPreset.isNullOrEmpty()){
                ...
                if (previousPreset.equals(args[1], true)){
                    ...
                    "Switched from $previousPreset to ${args[1]}"
                }
            }
            var message = TextComponent(previousString);
            message.clickEvent = ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/kb create ${sender.name}s${args[1]} $valuesDisplay");
        ...
        if (args.size == 1 && args[0] in KB_LIST.entries.map { it.name }){
            ...
            var section = plugin.config.getConfigurationSection("old-player-knockback.custom.configs")
            if (section == null) {
            ...
            for (knockback in section.getKeys(false)){
                var config = plugin.config.getConfigurationSection("old-player-knockback.custom.configs.$knockback")
                ...
                var valuesClean = config.getString("valuesDisplay")

                if (valuesClean.isNullOrEmpty()) {
                    continue
                }

                var message = TextComponent("$knockback: $valuesClean");
    ...
    override fun onTabComplete(
        ...
        if (args == null){
            sender.sendMessage("null args wtf??")
            return Collections.emptyList();
        }
        if (args.size == 1) {

            var configs = mutableListOf("list", "set", "add")
